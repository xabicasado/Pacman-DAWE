			...
			for (var r = row-1; r < row+2; r++) {
				for (var c = col-1; c < col+2; c++) {
					if((Math.abs(playerX - (c * thisGame.TILE_WIDTH)) < 8) && (Math.abs(playerY - (r * thisGame.TILE_HEIGHT)) < 8)) {
						pos = thisLevel.getMapTile(r, c);
						if(pos == 20) {
							if(player.velX > 0) player.x -= (thisGame.screenTileSize[1]-2)*thisGame.TILE_WIDTH;
							else player.x += (thisGame.screenTileSize[1]-2)*thisGame.TILE_WIDTH;

						} else if(pos == 21) {
							if(player.velY > 0) player.y -= (thisGame.screenTileSize[0]-2)*thisGame.TILE_HEIGHT;
							else player.y += (thisGame.screenTileSize[0]-2)*thisGame.TILE_HEIGHT;
						}
					}
				}
			}
